...
新版 dashboard 的“每日活动”折叠区会先展开，再只处理真实可见且未完成的链接；`rnoreward=1` 不能一律过滤，因为实测有些可见 daily 链接仍会到账。Bing quiz 会按可见题卡选择正确答案、点击“下一个/查看结果”，并以积分或任务状态变化为准。
...
- Microsoft 官方条款和支持页明确限制 bot / macro / automated methods；本项目不能保证账号不会被限制或封禁，也不会加入代理轮换、指纹伪装、验证码绕过、批量硬刷等规避风控功能
